...
import { Plus, CaretDown, CaretUp, Check, Users } from '@phosphor-icons/react'
...
export function AddExpenseModal({ isOpen, onClose, trip }) {
  ...
  const [splitMode, setSplitMode] = useState('everyone') // 'everyone' | 'custom'
  ...
  const handleSubmit = (e) => {
    ...
    try {
      const created = createExpense(payload)
      showToast(
        `Added "${created.description}" (${formatCurrency(created.amount, trip.currency)})!`,
        'success',
      )
      onClose()
    } catch (err) {
      if (err.validationErrors) {
        ...
        showToast(err.message || 'Could not create expense.', 'error')
      }
    } finally {
      setIsSubmitting(false)
    }
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Add Expense"
      description="Record a trip cost. Who paid and who shared it?"
    >
      {participants.length === 0 ? (
        <div className="text-center py-6 space-y-3">
          <div className="w-12 h-12 rounded-2xl bg-amber-100 text-amber-600 flex items-center justify-center mx-auto border-2 border-zinc-900 shadow-playful-sm">
          ...
          <p className="text-xs text-zinc-600 max-w-sm mx-auto">
            Please add the people joining this trip before logging expenses so we know who to split with.
          </p>
          <div className="pt-2">
        ...
        <form noValidate onSubmit={handleSubmit} className="space-y-4 pt-2">
          ...
          <Input
            ...
            label="What was it?"
            name="description"
            placeholder="e.g. Dinner, Petrol, Hotel, Scooters"
            value={description}
            onChange={(e) => {
            ...
            {splitMode === 'custom' && (
              <div className="p-3 bg-zinc-50 border-2 border-zinc-900 rounded-2xl space-y-2 animate-in fade-in duration-150">
                <span className="text-xs text-zinc-500 font-semibold block">
                  Check everyone who consumed or shared this bill:
                </span>
              ...
              <div className="space-y-3 pt-3 animate-in fade-in duration-150">
                ...
                <div className="w-full flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    name="note"
                    rows={2}
                    placeholder="e.g. Paid cash, 10% service charge included..."
                    value={note}
